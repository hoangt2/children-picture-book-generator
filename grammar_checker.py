...
def check_grammar(story_data, target_language="Finnish"):
    """
    Checks the grammar of the text in the story data using Gemini.
    Returns (is_valid, feedback).
    
    Args:
        story_data: The story JSON data
        target_language: The language to check (e.g., "Finnish", "French", "Spanish")
    """
    ...
    for page in story_data.get("pages", []):
        target_texts.append(f"Page {page.get('page_number')}: {page.get('text_target')}")
    
    full_text = "\n".join(target_texts)
    
    prompt = f"""
    You are a strict {target_language} grammar checker. Review the following story text ONLY for:
    1. Grammatical errors (case endings, verb conjugations, agreement, proper syntax)
...
if __name__ == "__main__":
    ...
    sample_story = {
        "pages": [
            {"page_number": 1, "text_target": "Minä olen kissa."}
        ]
    }
    valid, feedback = check_grammar(sample_story, "Finnish")
